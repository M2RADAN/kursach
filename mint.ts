...
RPC.addEventListener("utxos-changed", async (event: any) => {
  ...
  const removedEntry = event.data.removed.find(
    (entry: any) => entry.address.payload === address.toString().split(":")[1]
  );
  ...
  if (removedEntry) {
    ...
    log(
      `Added UTXO found for address: ${address.toString()} with UTXO: ${JSON.stringify(
        addedEntry,
        (key, value) =>
    ...
    log(
      `Removed UTXO found for address: ${address.toString()} with UTXO: ${JSON.stringify(
        removedEntry,
        (key, value) =>
          typeof value === "bigint" ? value.toString() + "n" : value
      )}`,
    ...
    log(
      `No removed UTXO found for address: ${address.toString()} in this UTXO change event`,
      "DEBUG"
    );
  }
});
...
for (let i = 0; i < loops; i++) {
  ...
  try {
    ...
    const commitTimeout = setTimeout(() => {
      if (!eventReceived) {
        log(
          "Timeout: Commit transaction did not mature within 2 minutes",
          "ERROR"
        );
        process.exit(1);
      }
    ...
    const { entries: newEntries } = await RPC.getUtxosByAddresses({
      addresses: [address.toString()],
    });
    ...
    log(
      `Main: Creating Transaction with revealUTX0s entries: ${revealUTXOs.entries[0]}`,
      "DEBUG"
    );
    ...
    for (const transaction of revealTransactions) {
      ...
      log(
        `Main: Transaction with revealUTX0s signed with ID: ${transaction.id}`,
        "DEBUG"
      );
    ...
    const revealTimeout = setTimeout(() => {
      if (!eventReceived) {
        log(
          "Timeout: Reveal transaction did not mature within 2 minutes",
          "ERROR"
        );
        process.exit(1);
      }
    ...
    const revealAccepted = updatedUTXOs.entries.some((entry) => {
      const transactionId = entry.entry.outpoint
        ? entry.entry.outpoint.transactionId
        : undefined;
